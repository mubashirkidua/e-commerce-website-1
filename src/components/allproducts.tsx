...
import { products, ProductDetail } from "./data/details2";
...
export default function Sidebar() {
  ...
  const kids = ["Girls", "Boys"];
  const priceRanges = ["Under ₹2500.00", "₹2501.00 - ₹5000.00"];
          ...
          <div className="space-y-2">
            {genders.map((gender, index) => (
              <div key={index} className="flex items-center">
                ...
                />
                <label htmlFor={gender.toLowerCase()} className="ml-2 text-slate-800 font-medium cursor-pointer">
                  {gender}
                </label>
              </div>
            ))}
            ...
            {kids.map((kid, index) => (
              <div key={index} className="flex items-center">
                <input
                  ...
                  className="form-checkbox h-4 w-4 text-slate-800 bg-slate-200 border-gray-300 rounded-sm focus:ring-slate-500"
                />
                <label htmlFor={kid.toLowerCase()} className="ml-2 text-slate-800 font-medium cursor-pointer">
                  {kid}
                </label>
              </div>
            ))}
            ...
            {priceRanges.map((price, index) => (
              <div key={index} className="flex items-center">
                <input
                  ...
                  className="form-checkbox h-4 w-4 text-slate-800 bg-slate-200 border-gray-300 rounded-sm focus:ring-slate-500"
                />
                <label htmlFor={`price-${index}`} className="ml-2 text-slate-800 font-medium cursor-pointer">
                  {price}
                </label>
              </div>
            ))}
